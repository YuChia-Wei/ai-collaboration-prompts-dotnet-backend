# Dev Workflow Deterministic Acceptance Oracle
...
natural-language request
  ...
  -> dev-workflow acceptance oracle
```
...
1. accept a preclassified high-level multi-stage software-development
   envelope;
2. activate `dev-workflow` without a named skill dependency;
...
```text
python .ai/scripts/tests/test_dev_workflow_acceptance.py -v
```

Run the activation oracle directly:

```text
python .ai/scripts/validate-dev-workflow-acceptance.py activation \
  --fixture .ai/assets/skills/dev-workflow/fixtures/acceptance/activation-high-level-pending-approval.yaml \
  --expected .ai/assets/skills/dev-workflow/fixtures/acceptance/activation-high-level-pending-approval.expected.yaml
```
