...
# ====================================================================
# Check Scripts Generator from Markdown (.NET)
#
...
echo ""
echo -e "${CYAN}Step 5: Updating compatibility copies...${NC}"

if [ "$DRY_RUN" = false ]; then
    # Create/update compatibility copies (avoid symlink issues on Windows)
    for script in "$OUTPUT_DIR"/check-*.sh; do
        if [ -f "$script" ]; then
            ...
            # Copy generated script for compatibility
            cp -f "$script" "$link_name"
            echo -e "${GREEN}✓ Copied $(basename "$link_name")${NC}"
        fi
    done
else
    echo -e "${YELLOW}⊖ Skipped (dry run)${NC}"
