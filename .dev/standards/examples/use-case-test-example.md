...
## Example: Create Iteration (Command)

```csharp
public sealed class CreateIterationTests
{
    private readonly IServiceScope _scope;
    private readonly ScenarioState _state = new();

    public CreateIterationTests(TestHostFixture host)
    {
    ...
    public void Rule_Successful_creation_create_sprint_successfully()
    {
        this.Given(_ => Given_valid_sprint_input())
            ...
            .And(_ => And_a_IterationCreated_event_is_published())
            .BDDfy();
    }
    ...
    void Given_valid_sprint_input()
    {
        ...
        _state.IterationId = Guid.NewGuid().ToString("N");
        _state.CreatorId = "test-creator";
        _state.Name = "Iteration 1";
    }

    async Task When_i_create_the_sprint()
    {
        var useCase = _scope.ServiceProvider.GetRequiredService<ICreateIterationUseCase>();
        var input = new CreateIterationInput(
            _state.ProductId!, _state.IterationId!, _state.Name!, _state.CreatorId!);
    ...
    void And_a_IterationCreated_event_is_published()
    {
        var collector = _scope.ServiceProvider.GetRequiredService<DomainEventCollector>();
        Assert.Equal(1, collector.CountOf<IterationCreated>());
    }
}
```
