...
Task artifact 以 `required_for_closeout` 明確區分哪些 selected levels 是
closeout gate。Unit 與 integration 為預設 selected levels；任何 conditional
level 都必須附 selection source。
...
Fresh session 必須從 Git、workflow locator、current task、target policy、
recorded test state 與已登錄的 handoff checkpoint 接續。Checkpoint 必須包含
非空的 exact next action，並設定 `hidden_context_required: false`；缺少或不一致
時不得以先前對話內容補足。
