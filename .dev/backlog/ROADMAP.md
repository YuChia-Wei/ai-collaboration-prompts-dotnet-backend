...
| `v0.5.0` | `in_progress` | yes | `PKG-003`, `SAG-001`, and `TOOL-001` are complete; `GOV-001`, `CAP-001`, and `VAL-001` are disposed. Complete `LANG-001`, `ENF-001`, `HANDOFF-001`, and `REL-001` as the remaining release blockers. | Continue from the verified package, adapter, offline dependency, and two-host portability checkpoints through language remediation and the integrating enforcement/release tasks. | [`2026-07-21-v0-5-0-development`](../workflows/2026-07-21-v0-5-0-development/workflow.yaml) |
