...
| `v0.5.0` | `release_ready` | yes | The four-source candidate, including exact automatic v0.4.2 upgrades, passes independent review, local and hosted gates, and the sanctioned pre-tag command on `main@540cd029`. | Await separate owner authorization before creating the exact annotated tag; publication remains a later governed phase and no tag exists yet. | [`2026-07-22-v0-5-0-pretag-portability-hotfix`](../workflows/2026-07-22-v0-5-0-pretag-portability-hotfix/workflow.yaml) |
...
If authorization is granted, use the exact command printed by the successful
pre-tag preparation on `main@540cd029`, then follow the governed tag,
