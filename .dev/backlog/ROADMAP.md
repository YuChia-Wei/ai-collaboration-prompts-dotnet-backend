...
| `v0.5.0` | `in_progress` | yes | Complete `PKG-003`, `SAG-001`, `ENF-001`, `TOOL-001`, `LANG-001`, `REL-001`, and `HANDOFF-001` as release blockers; `GOV-001` and `CAP-001` are disposed, while the selected `VAL-001` dependency/version implementation remains part of `V050-006`. | Inventory and disposition are complete; execute the frozen bounded release tasks with retained validation and commit checkpoints. | [`2026-07-21-v0-5-0-development`](../workflows/2026-07-21-v0-5-0-development/workflow.yaml) |
