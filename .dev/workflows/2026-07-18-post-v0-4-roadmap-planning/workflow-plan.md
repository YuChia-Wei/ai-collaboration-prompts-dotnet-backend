...
- Problem statement: the independently authored `post-v0.4.0-improvement-plan.md`, which was manually placed under historical assessment `ASM-20260717-004`, proposes a fixed `v0.4.1 -> v0.4.2 -> v0.5.0` sequence, while the user also wants a decision on assessment-template ownership and a later skill-family reorganization.
...
- Historical assessment input: `.dev/assessments/ASM-20260717-004/assessment.yaml`, whose locator and final report identify commit `82b88b7287deb7a64e0311fde6b1b53ea0d194b1`.
- Independent planning input: `.dev/backlog/plans/post-v0.4.0-improvement-plan.md`, authored separately by Claude Fable 5 against the v0.4.0-era repository and later placed manually in the assessment directory before this workflow corrected its ownership.
- Provenance split: the assessment remains historical; the Fable 5 document is a planning source rather than an assessment artifact. It does not record the exact analyzed full commit, assessment scope, method, and reproducible evidence in a successor locator.
- Required execution gate: confirm the exact v0.4.0 commit used by Claude Fable 5 and establish a successor assessment using the independent plan as primary input. Do not repeat the full audit by default; rerun only claims whose evidence cannot be pinned to that revision or has changed since.
- Codebase Memory MCP: session indexes were created as discovery accelerators; the final fast refresh produced 13,665 nodes and 19,517 edges. It excluded `.ai/assets`, `.ai/scripts`, `.claude`, and other material governance paths. A graph search found 41 files containing `repo-structure-sync`; direct inspection found 87 tracked files. The graph is an accelerator only under `AICTX-EVIDENCE-001`; no persisted cache is retained without the generated-inventory provenance contract.
...
| --- | --- | --- | --- | --- |
| 1 | `post-v0-4-successor-formalization` | `ai-context-auditor` | Confirm the exact v0.4.0 commit analyzed by Claude Fable 5. | Establish a governed successor assessment from the independent plan and reproducible evidence; rerun only unpinnable or stale claims. |
...
- Compact roadmap source: `.dev/backlog/ROADMAP.md`.
- Discovery entry: `.dev/backlog/INDEX.MD#active-release-roadmap`.
- The backlog roadmap owns release horizons, release-level state, activation gates, and workflow handoffs.
...
- Agents should read the backlog roadmap first and open this plan only when they need the decision record.
...
The governed validators and all task-specific tests pass. The broader
`.ai/scripts/tests` suite was also attempted: 143 of 145 tests passed in the
closest isolated environment. Two existing package-apply CLI tests failed
because their nested bare `python` command resolved to the Windows App
Execution Alias rather than the prepared virtual environment and therefore
could not import PyYAML. This reproduces the interpreter/dependency portability
gap already assigned to the v0.4.1 patch horizon; it is not treated as a pass
and is not remediated by this planning workflow.
...
| `ROADMAP-005` | Add target, completion, and publication release metadata to every backlog item and enforce the contract. | `completed` |
| `ROADMAP-006` | Clarify ASM-20260717-004 provenance and relocate the independently authored Fable 5 plan to the backlog planning-source area. | `completed` |

## Resume Checkpoint

- Last completed action: committed ASM-20260717-004 first, rebased the planning workflow onto that historical predecessor, promoted the compact roadmap to `.dev/backlog/ROADMAP.md`, added release metadata to all backlog items, recorded a provenance addendum, and moved the independent Fable 5 plan to `.dev/backlog/plans/`.
- Current task: none; planning workflow completed.
- Exact next action: read `.dev/backlog/ROADMAP.md`, confirm the exact v0.4.0 commit analyzed by Claude Fable 5, establish the successor assessment from the independent plan and reproducible evidence, and then create the `v0.4.1` remediation workflow.
- Validation already completed: direct file-backed policy/template/reference inspection; version-impact comparison; three bounded sub-agent reviews with main-agent verification; Codebase Memory exclusion comparison; 6 backlog release-contract GWT tests passed; assessment artifact validation passed for 7 assessments; all six task JSON files parsed; workflow artifact validation passed for 19 post-adoption workflows, 39 indexed workflow directories, and 11 backlog items; AI-context validation passed; Git commit validation and diff checks passed. The broader suite limitation is recorded above rather than claimed as a full pass.
...
- Blockers or unresolved decisions: release execution is gated on exact subject provenance and a governed assessment record, not automatically on a full re-audit. The user may later override the recommended shared locator ownership, but that would activate a `v0.5.0` contract-migration decision.

## Branch Lifecycle

| Segment | Branch | Base | Checkpoint Type | Commit | Remote / Target | Recorded At | Reason | Resume Branch / Action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `codex/2026-07-18-post-v0-4-roadmap-planning` | `claude/assessment/asm-20260717-004` | started-after-assessment | `2a31313ff2f3397fcb61af8b47f4e94de31f08f2` | local | `2026-07-18T14:19:06+08:00` | Reinitialize the unpushed planning workflow after committing its durable assessment predecessor. | Planning completed; read `.dev/backlog/ROADMAP.md` before activating the next workflow. |
